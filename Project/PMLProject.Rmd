...
predictRF <- predict(modelFitRF, validating, type="class")  #Predicting error using cross validation
...
```
An Accuracy of `r round(cmRF$overall['Accuracy'],5)` is NICE, especially considering that number is generated NOT for the training dataset but instead for the cross validation dataset.  Naturally, our error percentage would improve with repeated instances of the previous processes, but since there is inherent cross validation (k-fold) built into the Random Forest algorithm and also since we cross validated the model with a new dataset, validation, the improvements would be negligible.  Let's try another model though and see if we can improve on that error number.
...
predictCT <- predict(modelFitCT, validating)                  #Prediction via cross validation
cmCT <- confusionMatrix(predictCT, validating$classe)        #Accuracy
cmCT$overall['Accuracy']
table(predictCT, validating$classe)
```
After cross validating with the validation dataset gives us an accuracy of `r round(cmCT$overall['Accuracy'],2)` we can see this method's worse than random forest.  

Truth be told I don't think we can beat Random Forest's accuracy of `r round(cmRF$overall['Accuracy'],5)*100` percent.  The Random Forest model will be used to predict the 20 test cases from testing.  Even though the out-of-sample error can't be predicted exactly, Random Forest's in-sample error after cross validation was the lowest.  And since we cross validated the model with a validation data set instead of the training data set we expect the error to be very close but just a little higher due to out-of-sample error.  Hence we'll use the Random Forest model to predict the final 20 instances.
```{r}
